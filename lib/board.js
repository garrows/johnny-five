...
// the os module, so we read it from the file system instead
...
var isOnBoard = isGalileo || isRaspberryPi;
var boards = [];
var rport = /usb|acm|^com/i;
var port = "";

if (isOnBoard) {
  // TODO: Revisit this section with attention to extensibility.
  //
  if (isGalileo) {
...
function broadcast(err, type, io) {
  ...
  if (type === "ready") {
    ...
    if (!Repl.isBlocked && !process.browser) {
      process.stdin.emit("data", "1");
    }

    if (io.name !== "Mock" && !process.browser) {
 ...
 *
 */
Board.prototype.exit = function(level) {
    if (process.browser) {
        if (!level) {
            level = 0;
        }
        var result = confirm("Johnny-Five exited with level" + level + ". Close now?");
        if (result) {
            window.close();
        }
    } else {
        process.exit.apply(this, arguments);
    }
}
